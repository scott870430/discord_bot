...
### 報刀機器人使用
* ```!create_sheet```呼叫機器人創建本月最後五天的報刀sheet
* ```!url```呼叫機器人傳送google sheet連結
* ```!status```呼叫機器人回傳目前報刀狀況
* ```!status @戰隊成員```呼叫機器人回傳此成員目前報刀狀況
* ```!登記閃退```登記自己閃退
* ```!登記閃退 @戰隊成員```幫他人登記閃退
* ```!fill 傷害 boss```報刀，依序輸入傷害以及boss目標 1~5
* ```!fill 傷害 boss 殘```報殘刀，依序輸入傷害以及boss目標 1~5
* ```!fill @戰隊成員 傷害 boss```幫他人報刀，標記該成員，接著輸入傷害以及boss目標 1~5
## 執行機器人
